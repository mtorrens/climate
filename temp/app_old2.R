...
server <- function(input, output, session) {
  ...
  #   selectInput('zone', 'Country',
  #     c(sort(unique(cool.cities[which(cool.cities[, 'Zone'] == input$zone),
  #                               'FullCountry'])),
  #     '--Choose--'), '--Choose--')
  # )
  ...
  #   choices = c(sort(unique(cool.cities[which(cool.cities[, 'FullCountry'] == new.country()[1]),
  #                                           'DisplayName'])), 'Choose'),
  #   selected = 'Choose')
  # })
  ...
  #   selectInput('country_select', 'City',
  #     c(sort(unique(cool.cities[which(cool.cities[, 'FullCountry'] == input$country_select),
  #                               'FullCountry'])),
  #     '--Select--'), '--Select--')
  # )

    # new.country <- reactive({
    #   return(as.list(input$country_select))
    # })
    # observe({
    #   updateSelectInput(session, "city_select",
    #   choices = c(sort(unique(cool.cities[which(cool.cities[, 'FullCountry'] %in% new.country()),
    #                                           'DisplayName'])), 'Choose'),
    #   selected = 'Choose'
    # )})
  ...
  # output$city_select <- renderUI(
  #   selectInput("country_select", "City", 
  #     c(sort(unique(cool.cities[which(cool.cities[, 'FullCountry'] == input$country_select), 'DisplayName'])),
  #     "--Choose--"), "--Choose--")
  # )
  ...
  weather.plot <- eventReactive(input$click, {
    ...
    if (! file.exists(new.file)) {
      ...
      climate.plot(city.w, input$city, input$year, variable = input$variable)
    }
  })
